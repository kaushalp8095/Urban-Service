...
export default function Home() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="py-8 md:py-12 bg-gray-50 border-b">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 text-center">
            {[
              { icon: <Shield className="w-8 h-8 md:w-10 md:h-10 text-primary mb-2 md:mb-3" />, title: '100% Quality Assured', desc: "If you don't love our service, we'll make it right." },
              { icon: <Star className="w-8 h-8 md:w-10 md:h-10 text-primary mb-2 md:mb-3" />, title: 'Top Rated Pros', desc: 'Only the best professionals in the city.' },
              { icon: <Clock className="w-8 h-8 md:w-10 md:h-10 text-primary mb-2 md:mb-3" />, title: 'On-Time Delivery', desc: 'We respect your time. Punctuality guaranteed.' },
              { icon: <span className="text-primary text-2xl md:text-3xl font-bold mb-2 md:mb-3 block">₹</span>, title: 'Transparent Pricing', desc: 'No hidden charges. You pay what you see.' },
            ].map((b) => (
              <div key={b.title} className="flex flex-col items-center p-3">
                {b.icon}
                <h3 className="font-semibold text-sm md:text-base">{b.title}</h3>
                <p className="text-xs text-muted-foreground mt-1 hidden sm:block">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* App Banner */}
      <section className="py-12 bg-gradient-to-r from-blue-600 to-blue-800 text-white text-center px-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-3">Get the UrbanService App</h2>
        <p className="text-blue-100 mb-6 text-sm md:text-base">Book faster, track live, get exclusive app deals.</p>
        <div className="flex flex-col sm:flex-row justify-center gap-3">
          <Button variant="outline" className="text-white border-white hover:bg-white hover:text-blue-700 font-semibold">
            📱 Download for iOS
          </Button>
          <Button variant="outline" className="text-white border-white hover:bg-white hover:text-blue-700 font-semibold">
